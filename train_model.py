...
def get_args_parser():
    ...
    # Add the --model_type argument only when --type is 'vox'
...
def train_model(args):
    ...
    for step in range(start_iter, args.max_iter):
        ...
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_grad_l2_norm)
        optimizer.step()
        ...
        if (step % args.save_freq) == 0:
            ...
            print("Model saved at ", step, " successfully")
        elif (step == args.max_iter - 2):
            torch.save({
                'step': step,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()
        ...
        print("[%4d/%4d]; ttime: %.0f (%.2f, %.2f); loss: %.4f" % (step, args.max_iter, total_time, read_time, iter_time, loss_vis))
    print('Done!')
